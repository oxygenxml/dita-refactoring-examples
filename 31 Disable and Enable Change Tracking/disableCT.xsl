...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="contains($contentAfterRoot, '&lt;?oxy_options track_changes=&quot;on&quot;?&gt;')">
            <!-- Remove the processing instruction -->
            <xsl:value-of select="xrf:set-content-after-root(
                replace($contentAfterRoot, '&lt;\?oxy_options track_changes=&quot;on&quot;\?&gt;', ''))"/>
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>